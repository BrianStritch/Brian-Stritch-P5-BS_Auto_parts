    ...
    if 'q' in request.GET:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.error(
                    request,
                    'Update failed. Please ensure form data is correct.')
        else:
    ...
    def post(self, request):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.error(
                    request, 'Update unsuccesful.\
                         Please check your form details are correct.')
                return render(request, self.template_name, context)
...
class DeleteProfile(TemplateView):
    ...
    def get(self, request, pk):
        ...
        if request.user == user:
            ...
            messages.info(
                request,
                f'You are currently deleting profile for {user.username}')
            context = {
                'stop_toast_cart': True,
            }
            ...
            messages.error(request,
                           'Only the User can have access to this feature.')
