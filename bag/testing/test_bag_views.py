...
class TestBagViews(TestCase):
    ...
    def test_add_to_empty_bag_no_size(self):
        ...
        self.assertEqual(str(messages[0]), 'You have succesfully added "test name" to the shopping bag.')
    ...
    def test_add_to_empty_bag_with_size(self):
        ...
        response = self.client.post(f'/bag/add/{product.id}/',
                                    {"quantity": 1, "redirect_url": "view_bag",
                                     "product_size": 1})
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[0]), 'You have succesfully added "1" of our "test name" size "1" to the shopping bag.')
    ...
    def test_adjust_bag_quantity_to_two(self):
        ...
        self.assertEqual(str(messages[0]), 'You have succesfully edited the quantity of "test name" in your shopping bag.'
                         )
    ...
    def test_adjust_bag_quantity_to_zero(self):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[0]), 'You have succesfully removed "test name" from your shopping bag.')
    ...
    def test_remove_product_from_bag(self):
        ...
        self.assertEqual(bag, {})
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[1]), 'You have succesfully removed "test name" from your shopping bag.')
