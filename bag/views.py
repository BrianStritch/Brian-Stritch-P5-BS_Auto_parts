    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f'You have succesfully added another "{product.name}" size "{size}" to the shopping bag.')
            else:
                bag[item_id]['items_by_size'][size] = quantity
                messages.success(request, f'You have succesfully added "{product.name}" size "{size}" to the shopping bag.')
        else:
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(request, f'You have succesfully added "{quantity}" "{product.name}" size "{size}" to the shopping bag.')
    else:
        if item_id in list(bag.keys()):
...
def edit_bag(request, item_id):
    ...
    if size:
        if quantity > 0:
            bag[item_id]['items_by_size'][size] = quantity
            messages.success(request, f'You have succesfully edited the quantity of "{product.name}" size "{size}" in your shopping bag.')
        else:
