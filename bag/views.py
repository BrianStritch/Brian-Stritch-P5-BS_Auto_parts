    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request,
                    f'You have succesfully added "{product.name}" size "{size.upper()}" to the shopping bag.'
                )  # noqa
        else:
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(
                request,
                f'You have succesfully added "{quantity}" of our "{product.name}" size "{size.upper()}" to the shopping bag.'
            )  # noqa
    else:
        if item_id in list(bag.keys()):
